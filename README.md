...
# Publishes the core library artifact to the local Maven repository (usually located at ~/.m2/repository), allowing it to be used as a dependency in other local projects.

./gradlew publishCoreDebugPublicationToMavenLocal
# Publishes the debug library artifact to the local Maven repository (usually located at ~/.m2/repository), allowing it to be used as a dependency in other local projects.
```
